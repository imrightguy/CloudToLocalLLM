...
void main() {
  ...
  group('Auto Update Integration Tests', () {
    ...
    testWidgets('AutoUpdateService parses versions correctly', (WidgetTester tester) async {
      ...
      expect(components.major, equals(10), reason: 'Major version should be 10');
      expect(components.minor, equals(1), reason: 'Minor version should be 1');
      expect(components.patch, equals(200), reason: 'Patch version should be 200');
    });

    testWidgets('AutoUpdateService compares versions correctly', (WidgetTester tester) async {
    ...
    testWidgets('AutoUpdateService handles invalid version format', (WidgetTester tester) async {
    ...
    testWidgets('AutoUpdateService singleton pattern works correctly', (WidgetTester tester) async {
      // Verify singleton pattern - same instance returned
      final service1 = AutoUpdateService();
      final service2 = AutoUpdateService();

      expect(identical(service1, service2), isTrue,
        reason: 'AutoUpdateService should return the same instance (singleton pattern)');
    });
  });
}
