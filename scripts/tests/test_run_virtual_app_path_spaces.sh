...
LOG_FILE="$WORK_DIR/logs/virtual app.log"
PID_FILE="$WORK_DIR/state/virtual app.pid"
...
#!/bin/bash
set -euo pipefail
echo "app with spaces started on DISPLAY=$DISPLAY"
echo "app with spaces started on DISPLAY=$DISPLAY" >> "$APP_LOG"
sleep 60
EOF
chmod +x "$APP_PATH"
...
if ! grep -Fq '81 -screen 0 1366x768x24' "$XVFB_LOG"; then
  echo "Expected Xvfb to start with the requested display and resolution" >&2
  cat "$XVFB_LOG" >&2
  exit 1
fi

if ! grep -Fq 'app with spaces started on DISPLAY=:81' "$LOG_FILE"; then
  echo "Expected app output to land in the redirected log" >&2
  cat "$LOG_FILE" >&2
  exit 1
fi

if ! grep -Fq 'app with spaces started on DISPLAY=:81' "$APP_LOG"; then
  echo "Expected app output to land in the app-specific log" >&2
  cat "$APP_LOG" >&2
  exit 1
fi
...
echo "[test_run_virtual_app_path_spaces] Passed"
