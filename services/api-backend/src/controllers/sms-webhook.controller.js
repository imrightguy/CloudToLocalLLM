...
const handleIncoming = async (req, res) => {
  try {
    ...
    if (!From || !Body) {
      console.warn('⚠️  Incoming SMS webhook missing From or Body');
    ...
    if (employeeResult.success) {
      console.log(`✅ Employee reply processed: action=${employeeResult.action}, visit=${employeeResult.visitId}`);
    ...
    console.log(`ℹ️  Could not route incoming SMS from ${From}`);
    return res.status(200).type('text/xml').send('<Response></Response>');
  } catch (error) {
    console.error('❌ handleIncoming webhook error:', error.message);
...
const handleStatus = async (req, res) => {
  try {
    ...
    console.log(`📊 SMS status update: SID=${MessageSid} → ${SmsStatus}`);

    return res.status(200).send('OK');
  } catch (error) {
    console.error('❌ handleStatus webhook error:', error.message);
    return res.status(200).send('OK');
  }
};
  ...
  try {
    ...
    if (action === 'morning_reminder') {
      if (visitId) {
        ...
        console.log(`⏰ Morning reminder sweep: ${visits.length} visits`);
    ...
    if (action === 'post_survey') {
      if (visitId) {
        ...
        console.log(`📝 Post-visit survey sweep: ${visits.length} visits`);
    ...
    console.error('❌ handleSchedule error:', error.message);
