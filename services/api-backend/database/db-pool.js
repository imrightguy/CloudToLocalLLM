...
export function initializePool() {
  ...
  if (process.env.NODE_ENV === 'test' && !process.env.DB_HOST) {
    ...
    const generateId = () => 'mock-' + (++mockIdCounter) + '-' + Math.random().toString(36).substr(2, 5);
    ...
    const mockQuery = async (sql, params = []) => {
      ...
      if (isCountQuery(trimmed)) {
        ...
        conditions.forEach(cond => {
          if (cond.paramIndex < params.length) {
            rows = rows.filter(r => r[cond.column] === params[cond.paramIndex]);
          }
        });
      ...
      if (/^SELECT\s+/i.test(trimmed)) {
        ...
        const orderMatch = trimmed.match(/ORDER\s+BY\s+(\w+)(?:\.\w+)?\s+(ASC|DESC)/i);
        if (orderMatch) {
        ...
        if (selectCols && rows.length > 0) {
          rows = rows.map(r => {
            ...
            selectCols.forEach(col => {
              projected[col.alias] = r[col.expr] !== undefined ? r[col.expr] : null;
            });
            return projected;
          });
        }
      ...
      if (/^UPDATE\s+/i.test(trimmed)) {
        ...
        const setMatch = trimmed.match(/SET\s+(.+?)(?:\s+WHERE|\s+RETURNING|\s*;?\s*$)/i);
        ...
        if (setMatch) {
          ...
          parts.forEach(part => {
            ...
            if (assign) {
              setAssignments.push({ column: assign[1], expression: assign[2].trim() });
            }
          });
        }
        ...
        store.forEach(row => {
          let match = true;
          conditions.forEach(cond => {
            if (cond.paramIndex < params.length && row[cond.column] !== params[cond.paramIndex]) {
              match = false;
            }
          });
          if (match) {
            ...
            setAssignments.forEach(sa => {
              ...
              const incrementMatch = sa.expression.match(/^(\w+)\s*\+\s*(\d+)$/);
              if (incrementMatch) {
                const srcCol = incrementMatch[1];
                const inc = parseInt(incrementMatch[2], 10);
                row[sa.column] = (typeof row[srcCol] === 'number' ? row[srcCol] : 0) + inc;
              } else {
