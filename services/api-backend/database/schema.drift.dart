...
class BackendUsers extends Table {
  ...
  TextColumn get metadata => text().withDefault(const Constant('{}'))(); // JSON as text
...
class BackendAuditLogs extends Table {
  ...
  TextColumn get details => text().withDefault(const Constant('{}'))(); // JSON as text
  TextColumn get ipAddress => text().nullable()();
  TextColumn get userAgent => text().nullable()();
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
  ...
  @override
  List<String> get customConstraints => ["CHECK (severity IN ('debug', 'info', 'warn', 'error', 'critical'))"];
}
...
class BackendUserPreferences extends Table {
  ...
  TextColumn get preferences => text().withDefault(const Constant('{}'))(); // JSON as text
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
  DateTimeColumn get updatedAt => dateTime().withDefault(currentDateAndTime)();
...
class BackendMessages extends Table {
  TextColumn get id => text()();
  TextColumn get conversationId => text().references(BackendConversations, #id)();
  ...
  @override
  List<String> get customConstraints =>
      ["CHECK (role IN ('user', 'assistant', 'system'))", "CHECK (status IN ('sending', 'sent', 'error'))"];
}
