...
This document describes the log aggregation support implementation for the Zoidbot API backend. The implementation provides comprehensive support for log aggregation systems like Loki and ELK (Elasticsearch, Logstash, Kibana).
...
ELK_INDEX=zoidbot-api
ELK_INDEX_PATTERN=zoidbot-api-%DATE%
ELK_BATCH_SIZE=100
...
```
{service="zoidbot-api", level="error"}
```
...
```
service: "zoidbot-api" AND level: "error"
```
